...
RegisterNetEvent('dj_sile:createMusicMenu', function()
    lib.registerContext({
        id = 'music_menu',
            ...
            options = {
                {
                    ...
                    event = 'dj_sile:playMusicFromPlaylist'
                },
                {
...
RegisterNetEvent('dj_sile:playMusicFromPlaylist', function (data)
    local input = data.music
    if input then
        TriggerServerEvent('dj_sile:playMusic', input)
    end
end)

RegisterNetEvent('dj_sile:playMusicMenu', function (YoutubeURL)
    local input = lib.inputDialog(Config.Language['songSel'], {Config.Language['url']})
    if input then
        local YoutubeURL = input[1]
        TriggerServerEvent('dj_sile:playMusic', YoutubeURL)
    end
end)

RegisterNetEvent('dj_sile:changeVolumeMenu', function ()
    local input = lib.inputDialog(Config.Language['musicVolume'], {Config.Language['musicVolumeNm']})
    if input then
        local volume = input[1]
        TriggerServerEvent('dj_sile:changeVolume', volume)
    end
end)
            ...
            if CurrentAction ~= nil then
                ...
                if IsControlPressed(1, 38) then
                    ...
                    if CurrentAction == 'musicMenu' then
                        TriggerEvent('dj_sile:createMusicMenu')
                    end
                end
            end
            Wait(sleep)
